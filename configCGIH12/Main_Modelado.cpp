...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//la alargo
		model = glm::scale(model, glm::vec3(2.0f, 0.1, 2.0f));
		...
		/*patas(modelLoc, 0.8f, 0.5f, 0.8f, 0.2f, 1.0f, 0.2f);*/
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
		 rot -= 0.4f;
	 if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)  //rotacion x
		 rotx += 0.4f;
	 if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
		 rotx -= 0.4f;
	 if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS)  //rotacion z
		 rotz += 0.4f;
 ...
 void patas(GLint modelLoc, float x, float y, float z, float a, float b, float c) {
