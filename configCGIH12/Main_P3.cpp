...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShader.Use();
		glm::mat4 model=glm::mat4(1); //objeto
		glm::mat4 view=glm::mat4(1); //pantalla
		...
		//view = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 2,-700.0f ) ); // use with orthographic projection, movimos la camara
		
		 //var que mandan matrices a los vertex shader, con uniform
		...
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, 6.0f, 0.0f)); 
		...
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, 0.0f, 4.0f)); 
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f)); 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); 
		glDrawArrays(GL_TRIANGLES, 0, 36); 


		glBindVertexArray(0); //hasta el final
